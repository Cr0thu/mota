...
around two complementary approaches.  First, agent-assisted reference-route
...
no-demonstration variant remains incomplete, suggesting that local search priors
must be combined with archive-based exploration and continuation-aware stage
values.  A separate multi-agent reference planner, based on specialized
candidate scorers and beam search over legal simulator transitions, obtains a
validated trajectory with final HP \(436\).  This result is reported as a
reference-construction baseline rather than as evidence for no-demonstration
exploration.
\end{abstract}
...
This paper develops two complementary solution paths.  First, agent-assisted
reference-route construction is used to obtain high-quality trajectories and to
validate simulator semantics.  A multi-agent beam-search planner constructs a
strong reference trajectory with final HP \(436\), while an earlier
human-audited reference route with final HP \(403\) is retained for potential
learning and simulator audits.  Second, we build an AlphaGo Zero-style
learning-and-planning system that combines graph policy/value learning,
single-agent PUCT-MCTS, and potential-based reward shaping.  The reference-aided
policy/value variant also solves the first-ten-floor instance from the fixed
initial state, reaching final HP \(315\), whereas the no-demonstration variant
still reaches only intermediate milestones.  This gap motivates archive-based
exploration and continuation-aware stage values.
...
no-shop/no-fly constraints, and visualization consistency.  The fixed start
...
\[
  ...
  \mathrm{Money}_T=262.
\]
It uses no Floor-4 shop and no flying item.  The trajectory is retained as a
validation artifact, not as evidence that the pure RL system has solved the
task.
...
\begin{figure}[t]
  ...
  \caption{Summary of the main empirical signals.  Left: the learned potential
  ranks the reference transition within the top three legal alternatives in
  most validated states.  Middle: graph policy/value training reaches its best
  checkpoint before overfitting becomes visible.  Right: deterministic replay
  separates reference validation, reference-aided closed-loop search, and
  the current no-demonstration archive search.}
...
\[
  ...
  \mathrm{DEF}_T=27.
\]
The route passes the same deterministic replay and constraint checks as the
other reported routes.  Its milestone sequence is useful because it exposes the

...
def action_prior_logits(env, actions: list) -> np.ndarray:
    """Resource-aware action prior for the visualizer PPO policy.

    This is intentionally a logit prior, not reward shaping. It discourages
    local greedy moves such as expensive early fights or spending scarce keys on
    doors that do not reveal useful resources, while still leaving the neural
    policy free to choose them when training evidence supports it.
    """

    if not actions:
        return np.zeros(0, dtype=np.float32)
    from environment import Enemy, Item, Terrain, NPC

    priors = np.zeros(len(actions), dtype=np.float32)
    before_targets = _valuable_targets(env)
    for index, action in enumerate(actions):
        score = 0.0
        if isinstance(action, Item):
            score += _item_prior(env, action)
        elif isinstance(action, Enemy):
            score += _enemy_prior(env, action, before_targets)
        elif isinstance(action, Terrain):
            score += _terrain_prior(env, action, before_targets)
        elif isinstance(action, NPC):
            score += 3.5
        elif env.n2p.get(action) in env.env_data['floors'].get('afterEvent', {}):
            score += 4.0
        priors[index] = float(np.clip(score, -8.0, 8.0))
    return priors


def _item_prior(env, action) -> float:
    item_id = getattr(action, 'id', '')
    p = env.player
    if item_id in {'sword1', 'sword2', 'shield1', 'shield2'}:
        return 7.5
    if item_id in {'redGem', 'blueGem'}:
        return 4.5 + _gem_tactical_bonus(env, item_id)
    if item_id == 'redKey':
        return 7.0
    if item_id == 'blueKey':
        return 3.2 if p.items.get('blueKey', 0) < 2 else 1.0
    if item_id == 'yellowKey':
        return 2.0 if p.items.get('yellowKey', 0) < 5 else 0.7
    if 'Potion' in item_id or 'potion' in item_id:
        return 2.0 if p.hp < 320 else 0.4
    return 0.8


def _enemy_prior(env, action, before_targets: set) -> float:
    damage = _predict_damage(env, action)
    if damage is None:
        return -8.0
    p = env.player
    unlock_value = _unlock_value(env, action, before_targets)
    score = unlock_value
    score -= min(float(damage), 1200.0) / 90.0
    if damage <= 30:
        score += 1.2
    if getattr(action, 'id', '') in {'yellowGuard', 'skeletonCaptain'}:
        score += 5.0
    if p.atk <= 10 and damage > 45:
        score -= 4.0
    if p.def_ <= 10 and damage > 180:
        score -= 3.0
    if unlock_value <= 0.0 and damage > 50:
        score -= 3.5
    return score


def _terrain_prior(env, action, before_targets: set) -> float:
    terrain_id = getattr(action, 'id', '')
    if terrain_id in {'upFloor', 'downFloor', 'background'}:
        return 0.0
    if 'Door' not in terrain_id:
        return 0.0
    unlock_value = _unlock_value(env, action, before_targets)
    key_cost = {
        'yellowDoor': 1.6,
    ...
    score = unlock_value - key_cost
    if unlock_value <= 0.0:
        score -= key_cost
    p = env.player
    if terrain_id == 'yellowDoor' and p.items.get('yellowKey', 0) <= 1:
        score -= 1.5
    if terrain_id == 'blueDoor' and p.items.get('blueKey', 0) <= 1:
        score -= 2.5
    if terrain_id == 'redDoor' and p.items.get('redKey', 0) <= 1:
        score -= 4.0
    return score


def _unlock_value(env, action, before_targets: set) -> float:
    messages = list(getattr(env, 'last_messages', []))
    old_simulating = getattr(env, '_simulating', False)
    env._simulating = True
    stepped = False
    try:
        ...
        if ending != 'continue':
            return -6.0
        after_targets = _valuable_targets(env)
        new_targets = after_targets - before_targets
        return sum(_target_value(target[-1], env.player.hp) for target in new_targets)
    except Exception:
        return -6.0
    finally:
        if stepped:
            try:
                ...
                pass
        env._simulating = old_simulating
        env.last_messages = messages


def _valuable_targets(env) -> set:
    old_simulating = getattr(env, '_simulating', False)
    messages = list(getattr(env, 'last_messages', []))
    env._simulating = True
    targets = set()
    try:
        for action in env.get_feasible_actions():
            ...
            if pos is None:
                continue
            label = getattr(action, 'id', '')
            if label in {
                'sword1', 'sword2', 'shield1', 'shield2',
                'redGem', 'blueGem', 'yellowKey', 'blueKey', 'redKey',
            } or 'Potion' in label or 'potion' in label:
                targets.add((*pos[:3], label))
            elif pos[:3] in env.env_data['floors'].get('afterEvent', {}):
                targets.add((*pos[:3], 'event'))
    finally:
        env._simulating = old_simulating
        env.last_messages = messages
    return targets
...
def _target_value(label: str, hp: int) -> float:
    if label in {'sword1', 'sword2', 'shield1', 'shield2'}:
        return 7.5
    if label in {'redGem', 'blueGem'}:
        return 5.0
    if label == 'redKey':
        return 7.0
    if label == 'blueKey':
    ...
    if label == 'yellowKey':
        return 2.0
    if 'Potion' in label or 'potion' in label:
        return 2.0 if hp < 320 else 0.8
    if label == 'event':
        return 3.0
    return 0.0


def _gem_tactical_bonus(env, item_id: str) -> float:
    # Approximate marginal damage reduction against currently reachable enemies.
    bonus = 0.0
    for action in env.get_feasible_actions():
        if getattr(action, 'class_', '') != 'enemies':
            continue
        current = _predict_damage(env, action)
        if current is None:
            continue
        if item_id == 'redGem':
            env.player.atk += 1
            improved = _predict_damage(env, action)
            env.player.atk -= 1
        else:
            env.player.def_ += 1
            improved = _predict_damage(env, action)
        ...
        if improved is not None:
            bonus += max(0.0, float(current - improved)) / 120.0
    return min(2.5, bonus)


def _predict_damage(env, enemy) -> int | None:
    p = env.player
    p_damage = p.atk - getattr(enemy, 'def_', 0)
    if p_damage <= 0:
        return None
    rounds = enemy.hp // p_damage - (enemy.hp % p_damage == 0)
    e_damage = max(enemy.atk - p.def_, 0)
    damage = e_damage * rounds
    if enemy.special == 1:
        damage += e_damage
    elif enemy.special == 11:
        damage += p.hp // enemy.skw['value']
    elif enemy.special == 22:
        damage += enemy.skw['damage']
    damage -= p.mdef
    return max(0, min(int(damage), p.hp))
...
class PPO:
    ...
    def choose_action(self, env, actions: list) -> Tuple[object, dict]:
        ...
        prior_np = action_prior_logits(env, actions)
        ...
        prior_t = torch.from_numpy(prior_np).to(self.device, non_blocking=True)
    ...
    def greedy_action(self, env, actions: list):
        ...
        prior_t = torch.from_numpy(action_prior_logits(env, actions)).to(self.device, non_blocking=True)
    ...
    def update(self):
        ...
        priors_list = [
            tr.get('action_priors', np.zeros(tr['action_features'].shape[0], dtype=np.float32))
        ...
        for _ in range(self.update_epochs):
            ...
            for start in range(0, n, self.batch_size):
                ...
                b_priors = [priors_list[i] for i in bi]
                ...
                for j, feat in enumerate(b_feats):
                    ...
                    prior_t = torch.from_numpy(b_priors[j]).to(self.device, non_blocking=True)
                    ...
                    scores = self.network.actor(actor_input).view(-1) + prior_t
                    dist = Categorical(logits=scores)
...
def compute_sword_reward(env, base_reward: float, ending: str, action) -> float:
    ...
    # 楼梯只是宏动作路径里的跨楼层连接边，不是资源管理目标。
    # 不给上楼奖励，也不惩罚下楼；循环问题交给 action mask / 路径生成处理。
    if getattr(action, 'id', '') in {'upFloor', 'downFloor'}:
        return reward

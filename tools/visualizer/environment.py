            ...
            for effect, value in self.effects.items():
                if effect == 'hp':
                    if player.maxhp >= 0:
                        ...
                        self.re_hp = value  # maxhp < 0 表示無上限
...
class NPC(Node):
    ...
    def activate(self, player: Player) -> str:
        ...
        for command in self.commands:
            ...
            # 增加數值
                ...
                self._last_executed_commands.append(command)
            # 增加道具
    ...
    def re_activate(self, player: Player):
        self.activated = False
        for command in self._last_executed_commands[::-1]:  # 反向迭代
            # 數值
...
class Terrain(Node):
    ...
    # ------------------------------------------------------------------------
    def activate(self, player: Player) -> str:
        if self.activated and self.no_pass:
            raise RuntimeError('This node has been activated')
        self._activation_history.append(self.activated)
        self.activated = True
        # 地形處理
        if self.no_pass:
            ending = 'stop'
    ...
    def re_activate(self, player: Player):
        if self.activated:
            previous_activated = self._activation_history.pop() if self._activation_history else False
            self.activated = previous_activated
            if self.no_pass:
                if self.id == 'yellowDoor':
...
class Mota:
    ...
    def __init__(self):
        ...
        self.teleport_indices = set()  # 劇情傳送自動加入 observation 的索引集合
        self.last_messages = []       # 最近一次事件产生的剧情文本，供 UI 显示
        self._simulating = False      # get_feasible_actions 内部试走时不弹剧情
        self.re_after_event_snapshots = {}
    ...
    def build_env(self, env_name: str) -> int:
        ...
        change_targets = set(self.env_data['floors'].get('changeFloor', {}).values())
        ...
        for z, y, x in self.pos_iter():
            ...
            if kw['cls'] != 'except':  # 非事件id不須建立成節點物件
                ...
                self.n2p[node] = pos
                self.p2n[pos] = node
            elif pos in self.env_data['floors']['afterEvent'] or pos in change_targets:
                # 為帶有 afterEvent 或樓梯落點的背景格創建可通行節點。
        ...
        for f in range(0, layer):
            ...
            for i in range(0, width):
                for j in range(0, height):
                    ...
                    if pos in self.p2n:
                        node = self.p2n[pos]
                        if node.activated and node.class_ == 'terrains' and 'Door' in node.id:
                            continue
    ...
    def create_nodes(self) -> int:
        ...
        for node in self.n2p:
            ...
            # 若傳送後的起始位置本身就有節點，需建立鏈結
            if node_pos in self.p2n:
                _node = self.p2n[node_pos]
                node.links.append(_node)
                count += 1
            while q:
                ...
                for d in direction:
                    ...
                    # 與該事件節點建立鏈結
                        ...
                        visited.add(pos)
                        count += 1
                        # 對於 background 等可穿透節點，繼續 BFS
                        _node = self.p2n[pos]
                        if isinstance(_node, Terrain) and not _node.no_pass and _node.id == 'background':
                            q.append(pos)
        return count
    ...
    def reset(self):
        ...
        for node, pos in self.n2p.items():
            ...
            if isinstance(node, Terrain):
                node._activation_history.clear()
            if pos in disable:
                node.disabled = True
        ...
        self.teleport_indices.clear()
        self.last_messages.clear()
        self.re_after_event_snapshots.clear()
    ...
    def step(self, action: Node, return_reward: bool = False) -> (str, int):
        ...
        if self.n2p[action] in self.env_data['floors']['afterEvent']:
            commands = self.env_data['floors']['afterEvent'][self.n2p[action]]
            if any(command['type'] in {'set_player', 'teleport'} for command in commands):
                self.re_after_event_snapshots[action] = {
                    'lv': self.player.lv,
                    ...
                    'exp': self.player.exp,
                    'items': self.player.items.copy(),
                }
            for command in commands:
                if command['type'] == 'open':
                    if command['loc'] in self.p2n:
                    ...
                    if not self._simulating:
                        self.last_messages.append(command['text'])
                elif command['type'] == 'set_player':
                    ...
                    target_pos = command['loc']
                    if target_pos not in self.p2n:
                        node = Terrain({'cls': 'terrains', 'id': 'background', 'noPass': False})
                        node.disabled = True
                        node.activated = True
                        self.n2p[node] = target_pos
                        self.p2n[target_pos] = node
                        for n in self.n2p:
                            n.links.clear()
                        self.create_nodes()
                    pending_teleport_node = self.p2n[target_pos]
                    if isinstance(pending_teleport_node, Terrain) and pending_teleport_node.id == 'background':
                        pending_teleport_node.activated = True
                ...
                elif command['type'] == 'set_flag':
                    name = command['name']
                    val = command.get('value', 1)
                    self.flags[name] = self.flags.get(name, 0) + val
                elif command['type'] == 'if_flag':
                    val = self.flags.get(command['name'], 0)
                    cond = command.get('condition', 'True').strip()
                    triggered = False
                    for op in ['>=', '<=', '==', '>', '<', '!=']:
                        if cond.startswith(op):
                            ...
                            break
                    if triggered:
                        for subcmd in command.get('true', []):
                            if subcmd['type'] == 'open':
                                if subcmd['loc'] in self.p2n:
                                    self.p2n[subcmd['loc']].activated = True
                            elif subcmd['type'] == 'enable':
                                if subcmd['loc'] in self.p2n:
                                    self.p2n[subcmd['loc']].disabled = False
                            elif subcmd['type'] == 'set_flag':
                                self.flags[subcmd['name']] = self.flags.get(subcmd['name'], 0) + subcmd.get('value', 1)
                elif command['type'] == 'set_block':
                    pos = command['loc']
                    new_val = command['number']
                    z, y, x = pos
                    # 記錄原始值（僅第一次）
                    if pos not in self.re_set_blocks:
                        self.re_set_blocks[pos] = self.env_data['floors']['map'][z][y][x]
                    # 修改地圖數值
                    self.env_data['floors']['map'][z][y][x] = new_val
                    # 移除舊節點（如果存在）
                    if pos in self.p2n:
                        old_node = self.p2n[pos]
                        del self.p2n[pos]
                        del self.n2p[old_node]
                    # 若新值是事件格則創建新節點
                    kw = self.env_data['maps'].get(new_val)
                    if kw and kw['cls'] != 'except' and kw['id'] != 'player':
                        if kw['cls'] == 'enemies':
                            node = Enemy(kw)
                            node.reset(self.env_data['enemies'][kw['id']])
                        elif kw['cls'] == 'items':
                            node = Item(kw, self.env_data['items'].get(kw['id'], {}))
                        elif kw['cls'] == 'terrains':
                            node = Terrain(kw)
                        elif kw['cls'] == 'npcs':
                            node = NPC(kw, [])
                        elif kw['id'] == 'end':
                        ...
                        if node is not None:
                            self.n2p[node] = pos
                            self.p2n[pos] = node
                    # 若該位置有 afterEvent 且變為空地，創建 background 節點以便觸發事件
                    elif pos in self.env_data['floors']['afterEvent']:
                        node = Terrain({'cls': 'terrains', 'id': 'background', 'noPass': False})
                        self.n2p[node] = pos
                        self.p2n[pos] = node
                    # 重新計算所有節點的 links
        ...
        if pending_teleport_node is not None and pending_teleport_node is not action:
            self.observation.append(pending_teleport_node)
            self.teleport_indices.add(len(self.observation) - 1)
        else:
    ...
    def _activate_pair_stair(self, action: Node):
        ...
        if not isinstance(target_node, Terrain):
            return
        if target_node.id not in ('upFloor', 'downFloor', 'background'):
            return
    ...
    def back_step(self, times: int):
        ...
        for _ in range(times):
            ...
            if idx_to_check in self.pair_indices or idx_to_check in self.teleport_indices:
                n_entries += 2
            else:
                n_entries += 1
        if n_entries <= 0:
        ...
        for action in self.observation[:-n_entries - 1:-1]:  # 反向迭代，該方法比reversed()還快
            ...
            if self.n2p[action] in self.env_data['floors']['afterEvent']:
                for command in self.env_data['floors']['afterEvent'][self.n2p[action]][::-1]:
                    if command['type'] == 'open':
                    ...
                    elif command['type'] in {'message', 'teleport'}:
                        pass
                    elif command['type'] == 'set_player':
                        pass
                    elif command['type'] == 'set_flag':
                        self.flags[command['name']] -= command.get('value', 1)
                    elif command['type'] == 'if_flag':
                        val = self.flags.get(command['name'], 0)
                        cond = command.get('condition', 'True').strip()
                        triggered = False
                        for op in ['>=', '<=', '==', '>', '<', '!=']:
                            if cond.startswith(op):
                                ...
                                break
                        if triggered:
                            for subcmd in command.get('true', [])[::-1]:
                                if subcmd['type'] == 'open':
                                    if subcmd['loc'] in self.p2n:
                                        self.p2n[subcmd['loc']].activated = False
                                elif subcmd['type'] == 'enable':
                                    if subcmd['loc'] in self.p2n:
                                        self.p2n[subcmd['loc']].disabled = True
                                elif subcmd['type'] == 'set_flag':
                                    self.flags[subcmd['name']] -= subcmd.get('value', 1)
                    elif command['type'] == 'update_enemies':
                        ...
                        pos = command['loc']
                        if pos in self.re_set_blocks:
                            z, y, x = pos
                            old_val = self.re_set_blocks[pos]
                            self.env_data['floors']['map'][z][y][x] = old_val
                            # 移除當前節點（如果存在）
                            if pos in self.p2n:
                                old_node = self.p2n[pos]
                                del self.p2n[pos]
                                del self.n2p[old_node]
                            # 重建舊節點
                            kw = self.env_data['maps'].get(old_val)
                            if kw and kw['cls'] != 'except' and kw['id'] != 'player':
                                if kw['cls'] == 'enemies':
                                    node = Enemy(kw)
                                    node.reset(self.env_data['enemies'][kw['id']])
                                ...
                                if node is not None:
                                    self.n2p[node] = pos
                                    self.p2n[pos] = node
                            # 若還原後是空地且該位置有 afterEvent，重建 background 節點
                            elif pos in self.env_data['floors']['afterEvent']:
                                node = Terrain({'cls': 'terrains', 'id': 'background', 'noPass': False})
                                self.n2p[node] = pos
                                self.p2n[pos] = node
                            del self.re_set_blocks[pos]
                            need_recreate = True
            # 觸發事件處理
            action.re_activate(self.player)
            if action in self.re_after_event_snapshots:
                snapshot = self.re_after_event_snapshots.pop(action)
                self.player.lv = snapshot['lv']
                self.player.maxhp = snapshot['maxhp']
                self.player.hp = snapshot['hp']
                self.player.atk = snapshot['atk']
                self.player.def_ = snapshot['def']
                self.player.mdef = snapshot['mdef']
                self.player.money = snapshot['money']
                self.player.exp = snapshot['exp']
                self.player.items = snapshot['items'].copy()
        # 若有 set_block 被還原，重新計算 links
        if need_recreate:
            for node in self.n2p:
                node.links.clear()
            self.create_nodes()
        ...
        self.teleport_indices = {i for i in self.teleport_indices if i < new_len}
    ...
    def get_actions(self) -> list:
        ...
        if not node.activated and not self._is_internal_path_node(node):
            actions.append(node)
        # 添加目前位置
        visited.add(node)
        ...
        is_pair_dest = (
            isinstance(node, Terrain)
            and (
                node.id in ('upFloor', 'downFloor')
                or ((len(self.observation) - 1) in self.pair_indices and node.id == 'background')
            )
        )
        if is_pair_dest:
            self._collect_same_floor_actions_from(self.n2p[node], actions, visited)
        else:
            q.append(node)
        while q:
            node = q.pop()
            for _node in node.links:
                ...
                if _node not in visited:
                    visited.add(_node)
                    if _node.activated or self._is_internal_path_node(_node):
                        q.append(_node)
                    elif not _node.disabled:
                        actions.append(_node)
        return self._apply_story_action_gates(actions)

    def _collect_same_floor_actions_from(self, start_pos: tuple, actions: list, visited_nodes: set):
        """Collect reachable actions on the physical current floor after stair teleport.

        Stair nodes have precomputed links that represent stepping onto the stair
        and changing floors. After a paired stair has just placed the player on
        the destination stair, those links point back to the previous floor. For
        action generation we need the opposite: walk from the current coordinate
        on the current floor and only offer the destination stair itself as the
        ...
        direction = [(0, 0, 1), (0, 1, 0), (0, 0, -1), (0, -1, 0)]
        ...
        while q:
            ...
            for d in direction:
                ...
                if pos in visited_pos:
                    continue
                visited_pos.add(pos)
                if pos[0] != floor or not (0 <= pos[1] < height and 0 <= pos[2] < width):
                    continue
                node = self.p2n.get(pos)
                if node is None:
                    if map_[pos[0]][pos[1]][pos[2]] == 0:
                        q.append(pos)
                    continue
                ...
                if node.activated:
                    q.append(pos)
                elif self._is_internal_path_node(node):
                    q.append(pos)
                elif not node.disabled:
                    actions.append(node)

    @staticmethod
    def _is_reusable_stair(node):
        return isinstance(node, Terrain) and node.id in ('upFloor', 'downFloor')

    def _is_internal_path_node(self, node):
        """Nodes used only to connect reachable macro targets.

        Stairs and plain change-floor landing backgrounds should be graph edges,
        not policy actions. Background tiles with afterEvent remain actions,
        because they are real story/机关 triggers.
        """

        if self._is_reusable_stair(node):
            return True
        if isinstance(node, Terrain) and node.id == 'background':
            return self.n2p.get(node) not in self.env_data['floors']['afterEvent']
        return False

    def _apply_story_action_gates(self, actions: list) -> list:
        """Force compulsory early story beats before exposing optional targets."""

        # In the original first-10-floor flow, after killing the three 1F slimes
        # the only valid macro target is the 3F demon event. Because stairs are
        # now internal path edges, BFS can see other 3F targets too; hide them
        # until flag:03 is set by the demon event.
        if not self.flags.get('03'):
            demon_pos = (2, 9, 5)
            demon = self.p2n.get(demon_pos)
            if demon is not None and demon in actions and not demon.activated and not demon.disabled:
                return [demon]
        return actions

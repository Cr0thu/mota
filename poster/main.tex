...
\begin{tikzpicture}[remember picture,overlay]
  ...
  \placeblock{1.10in}{31.20in}{21.80in}{%
    {\fontsize{27}{33}\selectfont\bfseries\color{white}Main conclusions}\\[-0.01in]
    {\color{accent}\rule{\linewidth}{1.0pt}}\\[0.20in]
    \begin{minipage}[t]{0.33\linewidth}
      {\fontsize{21}{26}\selectfont\bfseries\color{white}Verified objective}\\[0.07in]
      {\fontsize{16.2}{22.8}\selectfont\color{white!84}
      Success is defined only by exact replay of the game transition log.  The final
      state must trigger the terminal event and retain positive HP; shaped return and
      model confidence are diagnostic signals only.}
    \end{minipage}\hfill
    \begin{minipage}[t]{0.33\linewidth}
      {\fontsize{21}{26}\selectfont\bfseries\color{white}Search signal}\\[0.07in]
      {\fontsize{16.2}{22.8}\selectfont\color{white!84}
      Potential learning converts ordered successful states into a dense progress
      signal.  PUCT and archive search use this signal to prioritize promising
      resource states without changing the task objective.}
    \end{minipage}\hfill
    \begin{minipage}[t]{0.29\linewidth}
      {\fontsize{21}{26}\selectfont\bfseries\color{white}Benchmark use}\\[0.07in]
      {\fontsize{16.2}{22.8}\selectfont\color{white!84}
      The same simulator can expose symbolic states, natural-language descriptions,
      and rendered videos, enabling a common benchmark for planners and multimodal
      agents.}
    \end{minipage}\\[0.28in]
    {\color{white!28}\rule{\linewidth}{0.55pt}}\\[0.17in]
    {\fontsize{16.5}{23.0}\selectfont\color{white!88}
    \textbf{Experimental pipeline:}\quad
    simulator \(\rightarrow\) graph action interface \(\rightarrow\) potential model
    \(\rightarrow\) PUCT/archive search \(\rightarrow\) strict route replay.}\\[0.28in]
    ...
    \textbf{Released components:} deterministic simulator, macro-action environment,
    graph-state encoder, potential training logs, archive cells, trajectory logs,
    failure cases, route visualizer, report source, and poster source.}
  }
  ...
  \node[anchor=south west,text width=21.80in,align=left]
    ...
    Core message: learning is most useful as guidance for exact search; the final
    claim is always certified by deterministic replay.}};

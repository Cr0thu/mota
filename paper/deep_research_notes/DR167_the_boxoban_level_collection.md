# DR167 The Boxoban Level Collection

- Year: 2020
- Area: sokoban-planning
- URL: https://github.com/deepmind/boxoban-levels
- Experiment role: learned_search

## 问题
这篇工作被纳入精读，是因为它处理了长视野、稀疏反馈、组合搜索、奖励学习或示范数据使用中的一个关键环节。

## 方法
Sokoban 的不可逆操作和长程死锁对应魔塔的钥匙、HP、攻防临界；搜索 trace 比最终路线更有训练价值。

## 对魔塔的启示
dataset design and puzzle split methodology for deterministic planning games

## 工程取舍
如果进入当前前十层实验，优先转化为可测试的搜索、reward、ranker 或 ablation 模块；如果属于世界模型/LLM/大规模泛化方向，则先作为后续 50 层扩展参考。

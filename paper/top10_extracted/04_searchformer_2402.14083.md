...
Lucas Lehnert1, Sainbayar Sukhbaatar1, DiJia Su1, Qinqing Zheng1, Paul Mcvay1, Michael Rabbat1, Yuandong Tian1
...
![](images/b533d7a02a96132987b2539cec987669899106fd0010ed4f26d624fa15abd86d.jpg)
...
2. Non-deterministic $A ^ { * }$ dataset: Sequences are generated by executing $A ^ { * }$ in a non-deterministic fashion (by randomly ordering child nodes and breaking equal cost ties randomly). Consequently, given a task prompt, the optimal plan and $A ^ { * }$ execution trace is no longer unique and there are multiple correct responses. Here, the Transformer learns to generate the random tie breaking rules implicitly encoded in the sequence data. Consequently, the generated sequences vary between different executions, but the resulting plans are still optimal and execution traces still respect $A ^ { \ast } \mathrm { ^ { \circ } s }$ cost and heuristic calculations as described in Section 3.3.
...
<table><tr><td>Params.</td><td>Model</td><td>Solved (%)</td><td>Optimal (%)</td><td>SWC (↑)</td><td>ILR (solved, ↑)</td><td>ILR (optimal, ↑)</td></tr><tr><td rowspan="5">45M</td><td>Solution only</td><td>90.3 ±1.0</td><td>86.8 ±0.3</td><td>0.890 ±0.009</td><td>-</td><td>-</td></tr><tr><td>Search augmented</td><td>92.5 ±1.0</td><td>90.8 ±1.6</td><td>0.924 ±0.011</td><td>0.908 ±0.020</td><td>0.919 ±0.019</td></tr><tr><td>Searchformer, step 1</td><td>95.5 ±1.0</td><td>93.5 ±1.0</td><td>0.953 ±0.010</td><td>1.054 ±0.025</td><td>1.062 ±0.015</td></tr><tr><td>Searchformer, step 2</td><td>96.0 ±0.5</td><td>93.4 ±0.6</td><td>0.957 ±0.005</td><td>1.158 ±0.025</td><td>1.181 ±0.012</td></tr><tr><td>Searchformer, step 3</td><td>95.5 ±0.8</td><td>93.7 ±1.6</td><td>0.953 ±0.009</td><td>1.292 ±0.044</td><td>1.343 ±0.067</td></tr><tr><td rowspan="2">175M</td><td>Solution only</td><td>95.7 ±0.2</td><td>90.0 ±0.8</td><td>0.949 ±0.003</td><td>-</td><td>-</td></tr><tr><td>Search augmented</td><td>95.2 ±0.9</td><td>93.2 ±1.0</td><td>0.949 ±0.010</td><td>0.925 ±0.010</td><td>0.933 ±0.011</td></tr><tr><td>757M</td><td>Solution only</td><td>96.5 ±0.1</td><td>92.2 ±1.2</td><td>0.958 ±0.002</td><td>-</td><td>-</td></tr></table>
...
![](images/f636151dde405625b168dcde9057bf70116ab0434c0b39bb9382a1c3f1d929bf.jpg)  
(a) Search length improvement.

![](images/4953af713a3bc7990a118261e45e50871e036d7ffe3e1a88a2919e2a9cbe0c4d.jpg)  
...
A token sequence is processed by a neural network by first indexing the set of all tokens contained in a specific dataset. This indexing is used to map any token sequence to a set of integers. Formally, we denote a prompt as a sequence of integers $\boldsymbol { x } _ { 1 : n } = ( x _ { 1 } , . . . , x _ { n } )$ . An encoder Transformer neural network with network weights $\pmb { \theta } _ { \mathrm { e n c } }$ is a function $f _ { \pmb { \theta } _ { \mathrm { e n c } } }$ mapping a tokenized prompt $x _ { 1 : n }$ of arbitrary length n to a sequence of n real-valued vectors:

$$
f _ {\boldsymbol {\theta} _ {\mathrm{enc}}}: x _ {1: n} \mapsto \boldsymbol {z} _ {1: n} \tag {1}
$$

Each vector $z _ { i }$ in this sequence of vectors $z _ { 1 : n }$ has the same dimension. The decoder network is then used to generate a response auto-regressively: Starting with a specific beginning-of-sequence token bos to cue the decoder, a sequence is recursively built by first mapping the start token bos to a probability distribution over next-tokens. This probability distribution is stored as a vector ${ \pmb p } _ { 1 }$ whose dimension is equal to the size of the vocabulary. The next token is then generated by sampling from this distribution and the sampled token is appended to the response sequence. Subsequently the two-token response sequence is fed into the decoder again to compute the next probability vector $\pmb { p } _ { 2 }$ and sample the next token. This procedure is repeated until an end-of-sequence token eos is sampled. While only the last computed probability vector is needed to sample the next token, the decoder network simultaneously predicts a sequence of next token probability vectors $\pmb { p } _ { 1 : m }$ given an input sequence $y _ { 1 : m }$ . Furthermore, this prediction is conditioned on the encoder output $z _ { 1 : n }$ . The decoder Transformer neural network with weight parameters ${ \pmb \theta } _ { \mathrm { d e c } }$ is therefore a function

$$
g _ {\boldsymbol {\theta} _ {\mathrm{dec}}}: \boldsymbol {z} _ {1: n}, y _ {1: m} \mapsto \boldsymbol {p} _ {1: m}. \tag {2}
$$

The encoder network $f _ { \pmb { \theta } _ { \mathrm { e n c } } }$ and decoder network $g _ { \pmb { \theta } _ { \mathrm { d e c } } }$ internally both use a number of stacked causal attention layers to form an encoder-decoder Transformer (Vaswani et al., 2017) as outlined in more detail in Appendix B. We denote a concatenation of all encoder parameters $\pmb { \theta } _ { \mathrm { e n c } }$ and decoder parameters ${ \pmb \theta } _ { \mathrm { d e c } }$ with the vector θ.

# A.2 Training with teacher forcing

An encoder-decoder architecture is optimized to generate responses that follow the distribution of a training dataset by minimizing the cross-entropy between the training data distribution $p _ { \mathcal { D } }$ over prompt-response pairs $( x _ { n } , y _ { m } )$ and the distribution $p _ { \pmb { \theta } }$ with which the encoder-decoder model is generating responses. This cross-entropy loss objective

$$
H (p _ {\mathcal {D}}, p _ {\boldsymbol {\theta}}) = \mathbb {E} _ {\mathcal {D}} \left[ - \log p _ {\boldsymbol {\theta}} (y _ {1: m} | x _ {1: n}) \right] \tag {3}
$$

$$
= \mathbb {E} _ {\mathcal {D}} \left[ - \sum_ {i = 1} ^ {m - 1} \log p _ {\boldsymbol {\theta}} (y _ {i + 1: m} | y _ {1: i}, x _ {1: n}) \right], \tag {4}
$$
...
$$
L (\boldsymbol {\theta}) = \frac {1}{D} \sum_ {d = 1} ^ {D} \frac {1}{m _ {d} - 1} \sum_ {i = 1} ^ {m _ {d} - 1} \log p _ {\boldsymbol {\theta}} (y _ {i + 1: m _ {d}} ^ {d} | y _ {1: i} ^ {d}, x _ {1: n _ {d}} ^ {d}), \tag {5}
$$
...
![](images/0a58977c76bd72bf8b2292ff6699ba5c111d7c98e0438120e087d2e633c91088.jpg)  
...
Table 2 lists the used architecture hyper-parameter and Table 3 lists the hyper-parameters used for optimizing each model. All experiments were implemented in $\mathrm { P y }$ Torch 2.0 (Paszke et al., 2019) and default parameters were used unless reported here otherwise.
...
<table><tr><td>Parameter</td><td>15M model</td><td>46M model</td><td>175M model</td><td>747M model</td></tr><tr><td>Layers</td><td>6</td><td>8</td><td>9</td><td>16</td></tr><tr><td>Heads</td><td>3</td><td>4</td><td>4</td><td>12</td></tr><tr><td>Layer dim.</td><td>64</td><td>96</td><td>192</td><td>96</td></tr></table>

Table 3 Optimization hyper-parameters. Every model was optimized using AdamW (Loshchilov & Hutter, 2019) with setting $\beta _ { 0 } = 0 . 9$ and $\beta _ { 1 } = 0 . 9 9$ . Initially, the learning rate was linearly interpolated: Starting at zero and then increasing linearly to the value listed below until step 2000. Then a cosine learning rate schedule was followed (Loshchilov & Hutter, 2016). to the between zero at the first training step and the listed value below for the 

<table><tr><td>Parameter</td><td>Model</td><td>Maze Tasks</td><td>Sokoban Puzzels</td></tr><tr><td rowspan="4">Learning rate</td><td>15M</td><td> $2.5 \cdot 10^{-4}$ </td><td> $2.5 \cdot 10^{-4}$ </td></tr><tr><td>46M</td><td> $7.5 \cdot 10^{-5}$ </td><td> $7.5 \cdot 10^{-5}$ </td></tr><tr><td>175M</td><td> $5.0 \cdot 10^{-5}$ </td><td> $5.0 \cdot 10^{-5}$ </td></tr><tr><td>747M</td><td>-</td><td> $5.0 \cdot 10^{-5}$ </td></tr><tr><td>Batch size</td><td>all</td><td>16</td><td>64</td></tr><tr><td>Training steps</td><td>all</td><td>400000</td><td>80000</td></tr></table>

# C Dataset generation

All datasets were generated by first randomly sampling a task and then executing $A ^ { * }$ to obtain an optimal plan. Maze tasks were generated first by randomly selecting 30-50% of all cells to be wall cells. Then a start and goal location was randomly selected and $A ^ { * }$ was executed to obtain an optimal plan. If the plan had a length of at least the mazes width or height (e.g. for $1 0 \times 1 0$ mazes the optimal plan needs to contain at least 10 steps), then the task was added into the dataset. For Sokoban, a $7 \times 7$ grid was sampled and two additional wall cells were added as obstacles to the interior of the map. Then two docks, boxes, and the worker locations were randomly placed. If the sampled task is solvable by $\mathrm { A ^ { * } }$ , then the task was admitted to the dataset.

Due to the large volume of generated data, all data was stored in and transformed with a MongoDB (MongoDB Inc.) instance and map-reduce techniques. Furthermore, when sampling tasks, the dataset was constructed to reject duplicate tasks ensuring that training and test data and each prompt is distinct. Once each task and trace dataset was generated, each execution trace is converted into prompt and response token sequences, as illustrated in Figure 1(c) and Figure 8. Because the Sokoban tasks are very complex and difficult to solve for $\mathrm { A ^ { * } }$ , the resulting token sequences were partially very long and reached almost 100000 tokens. Due to GPU memory requirements, the Sokoban dataset was further sliced to only include sequences of with at most 10000 tokens. Figure 8 compares the sequence length distribution for each dataset. During training, each dataset was also sorted and sliced to only contains the reported number of training sequences. Furthermore, each model was evaluated on the same test tasks within each task dataset. The test dataset contains plans and traces that are of comparable length to the training data (Figure 8).

![](images/7a3eb47efa1a7f7ec8639138df228dc837dd38f5397de378f70d37bb2dbf8cae.jpg)  
...
$$
\text {Prompt} \left\{ \begin{array}{l l} \text {bos} & \\ \text {worker 2 3} & \\ \text {box 2 4} & \\ \text {box 3 4} & \\ \text {dock 1 3} & \\ \text {dock 4 4} & \\ \text {wall 0 0} & \\ \text {wall 0 1} & \\ \dots & \\ \text {wall 6 6} & \\ \text {eos} & \end{array} \right. \quad \text {Trace (2583 tokens)} \left\{ \begin{array}{l l} \text {bos} & \\ \text {create worker 2 3 box 2 4 box 3 4 c0 c3} & \\ \text {close worker 2 3 box 2 4 box 3 4 c0 c3} & \\ \dots & \\ \text {create worker 5 4 box 2 3 c10 c3} & \\ \text {close worker 2 1 c12 c0} & \end{array} \right. \quad \text {Plan} \left\{ \begin{array}{l l} \text {plan 2 3} & \\ \text {plan 1 3} & \\ \text {plan 1 4} & \\ \text {plan 1 5} & \\ \text {plan 2 5} & \\ \text {plan 2 4} & \\ \text {plan 3 4} & \\ \text {plan 2 4} & \\ \text {plan 2 3} & \\ \text {plan 2 2} & \\ \text {plan 1 2} & \\ \text {plan 1 1} & \\ \text {plan 2 1} & \\ \text {eos} & \end{array} \right.
$$
...
![](images/78309491d0e10372f06549162c8f46362945c0ded554573f89c06c2fc3aac52d.jpg)  
...
![](images/cf4454083a60c0b6bb2ffa5916dad2b324a2a5a908e4ef0d9bc9833b6126be7a.jpg)  
(a) Plan Sequence Length

![](images/2a09464e428cf9c925b9436b0dd502c59c2ec4215397eedf416844057f0526c1.jpg)  
...
• SWC score. To further measure the sub-optimalness of the resulting plans, we also report the Success Weighted $b y$ Cost (SWC) (Wu et al., 2019), a statistic that factors in how close the cost $l _ { i }$ of the best predicted correct plan (over 64 trials) is to the optimal plan cost $l _ { i } ^ { * }$ , averaged across all n test tasks and weighted by a binary variable $c _ { i } \in \{ 0 , 1 \}$ :

$$
\mathrm{SWC} := \frac {1}{n} \sum_ {i = 1} ^ {n} c _ {i} \frac {l _ {i} ^ {*}}{\max \left\{l _ {i} , l _ {i} ^ {*} \right\}}.
$$

When computing the SWC score, the binary variable $c _ { i }$ is set to one if a correct plan was found and zero otherwise. This SWC score lies between zero and one. If all generated sequences end in an optimal plan, then this value is one.
...
• ILR score. To further measure how much improvement of the model-generated search dynamics against $A ^ { * }$ planning, we report the Improved Length Ratio of Search Dynamics (ILR) score. Specifically, for each test task i, we compute the ratio between the length $t _ { i }$ of the shortest generated search dynamics sequence and the $A ^ { * }$ token sequence length $t _ { i } ^ { * }$ . We then average across all test tasks while only including ratios for tasks for which either an optimal or a correct (and potentially sub-optimal) plan was found, as specified by the binary variable $c _ { i }$ . The corresponding measures are thus called ILR-on-optimal and ILR-on-solved. The ILR is defined as

$$
\mathrm{ILR} := \frac {1}{n} \sum_ {i = 1} ^ {n} c _ {i} \frac {t _ {i} ^ {*}}{t _ {i}}.
$$
...
![](images/d6881191ba0eaed004bb6a76887aa40b5e3644e58da4916e59e7eef8faf5d0ad.jpg)

(a) Comparison of all sequences ending with an optimal plan   
![](images/eca63fe12a7a2b9e6329fcc5ebaabc01acd1d3c61085729a032b4de5f4b26761.jpg)

(b) Comparison of all sequences ending with a correct plan   
![](images/bda6301c6e90bc1eda7c38ff972e65674d9993c932a801525c35f16c86fcaffd.jpg)  
...
![](images/b28f750b65c0c3a3e9edd12578f0a96c60459ac9c6893f7865daae89c1c75619.jpg)  
